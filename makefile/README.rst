...
to the desired compilers. Note that ``gcc/g++`` is the default compiler on Linux,
and ``clang`` is the default compiler on Mac OSX.
...
6. The ``udp_receiver`` binary (C example application for receiving encoded packets)
